    ...
    public partial class MainForm : Form
    {
        ...
        private void SetTextFormElements()
        {
            foreach (var item in _parameters.ParametersDict)
            {
                ...
                // Этот обработчик необходим, так как мы итерируемся по словарю
                // ВСЕХ параметров, а в _controls находятся не все параметры.
                // Если ручка цилиндрическая, то в _controls не будет содержаться
                // ее длина и ширина. Следовательно, когда в цикле мы дойдем до
                // ширины ручки, программа попытается найти ее в _controls, а
                // так как ее нет, выбросится исключение KeyNotFoundException.
